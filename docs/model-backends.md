...
同一套閘道也能餵給 Claude Code 以外的用戶端，那是 LiteLLM 本來的用途，但能不能用取決於下游怎麼呼叫，見「不只 Claude Code」那節。
...
## 不只 Claude Code：拿它當一般 OpenAI 相容閘道

前面整份講的是把 Claude Code 接到這個後端。但 LiteLLM 本來就是通用閘道，對外那一層是 OpenAI 相容介面，所以任何吃 OpenAI 金鑰的用戶端都能指過來，訂閱額度等於變成一把到處插得上的金鑰。

**這條只適合本機除錯。** 理由在下面第三張表。

### 接法

- 閘道發出來的金鑰（`general_settings.master_key`，或另開的 virtual key）對下游就是一把 OpenAI 金鑰。除錯用 master key 就夠，要做用量歸戶或分權才需要 virtual key。
- 下游改兩處：`base_url` 指到 `http://127.0.0.1:4000/v1`、金鑰換成上面那把。
- ⚠️ **下游必須允許改 `base_url`**。只換金鑰沒用，把 `api.openai.com` 寫死的用戶端接不上。

### 三個能力面的洞

介面相容不等於能力相容。閘道能把請求轉成 OpenAI 的形狀，但變不出後端沒有的東西。

| 洞 | 症狀 | 誰會中 |
|---|---|---|
| **沒有嵌入端點** | 訂閱端點只有 `.../codex/responses` 這一條，沒有對應的 embeddings | 任何要算向量的用途（檢索索引、語意搜尋） |
| **不收 system role** | `{"detail":"System messages are not allowed"}` | 幾乎所有應用，多數都會送系統提示。要靠前置 hook 折進 `instructions`，寫法見上一節 |
| **非串流會壞** | `Unknown items in responses API response: []` | 送 `stream: false` 的下游。「一次拿完整回應」那種用法全中，例如單題生成、分類、抽取 |

前兩個有解或可繞，第三個要嘛改下游走串流，要嘛在閘道層聚合。
...
|---|---|
| 憑證會過期、續期麻煩 | ✅ 過期重跑一次就好 |
| 訂閱條款的灰帶 | ✅ 自己機器、自己帳號、不對外提供服務 |
| 額度跟日常用的 CLI 共用 | ✅ |
| 後端隨時可能調整 | ✅ |
| **沒有嵌入端點** | ❌ 要測檢索就是打不出去 |
| **非串流會壞** | ❌ 跟用途無關，第一個請求就撞 |

前四條是穩定性與風險，除錯情境確實吞得下。**後兩條是能力面的洞，不會因為「只是除錯」就消失。**

### 憑證：要放著跑就讓它自己登入

複製一份攤平的憑證（見上面「憑證不能直接指向 Codex CLI 那份」）**只適合一次性測試**。沒帶 `refresh_token` 就沒辦法自己續期，效期一到整條斷。任何要持續跑一段時間的用途，走裝置碼登入讓它拿自己那一份。

### 還沒驗的

`/chat/completions` 進、`/responses` 出這條轉換沒有實測過。上面所有實測都是 Claude Code 走 `/v1/messages` 那條路。要餵給一般的 OpenAI 用戶端之前，這是第一個該驗的點，不是最後一個。
